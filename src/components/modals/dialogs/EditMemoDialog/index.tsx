...
import { SlideToDelete, SlideToUpdate } from '@/components/modals/ui/Slide';
...
export const EditMemoDialog: React.FC<BaseDialogProps> = ({ isOpen, onClose }) => {
  const { currentEditingMemo, refreshMemos } = useApp();
  ...
  useEffect(() => {
    if (currentEditingMemo) {
      setTitle(currentEditingMemo.title);
      setContent(currentEditingMemo.content);
      setColor(currentEditingMemo.color || '#FFFFFF');

      // 直接根据后端返回的style.code设置初始样式
      const style = getStyleByCode(currentEditingMemo.style.code);
      setStyleId(style.id);
    }
  }, [currentEditingMemo]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // 添加调试日志
      console.log('Updating memo with styleId:', styleId, 'styleCode:', styleCode);
      const requestBody = { title, content, color, styleCode };
      console.log('Request body:', requestBody);

      await fetch(API_ENDPOINTS.MEMO.UPDATE(currentEditingMemo.id), {
        method: 'PATCH',
      ...
      setStyleId(1);
      onClose();
      refreshMemos(); // 刷新memo列表
      return true;
    } catch (err: any) {
      setError('更新失败: ' + (err.message || err.toString()));
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      await fetch(API_ENDPOINTS.MEMO.DELETE(currentEditingMemo.id), {
        method: 'DELETE',
        credentials: 'include',
      })
        .then(async res => {
      ...
      setError('删除失败: ' + (err.message || err.toString()));
      return false;
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col gap-4 p-4">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          >
            ...
            <div className="relative z-10 h-full flex flex-col">
              ...
              <textarea
                ...
                placeholder={`内容最大长度${textLimits.content.totalUnits}单位`}
          ...
          {/* 视觉宽度计数 - 放在 demo 下边 */}
          <div className={`text-xs text-center ${contentVisualInfo.isOverLimit ? 'text-red-500' :
